...
// Handle Form Submission and Update Post Content
add_action('gform_after_submission_4', 'create_note_content', 10, 2);
function create_note_content($entry, $form) {
    // Retrieve the dynamically populated post ID from the hidden field
    $post_id = rgar($entry, '6'); // Assuming '2' is the ID of your hidden field

    // Retrieve the OpenAI response using the merge tag
    $openai_response = GFCommon::replace_variables('{openai_feed_3}', $form, $entry);
