...
// Ask Sensei Form
// Post Creation
add_action('gform_after_submission_40', 'create_pitch_sensei_post', 10, 2);

function create_pitch_sensei_post($entry, $form) {
    ...
    $post_content = '<div class="alert alert-info">'.$custom_1.'</div><div class="alert alert-info">'.$custom_2.'</div><div class="alert alert-info">'.$custom_3.'</div><div class="alert alert-info">'.$custom_4.'</div><div class="alert alert-info">'.$custom_5.'</div><div class="alert alert-info">'.$custom_6.'</div><div class="alert alert-info">'.$custom_7.'</div><div class="alert alert-info">'.$custom_8.'</div><div class="alert alert-success my-3 ml-5">'.$response.'</div>';

    // Fetch the current user ID
    $current_user_id = get_current_user_id();

    // Save each custom field to its respective ACF field as user metadata
    update_field('my_pitch_1', $custom_1, 'user_' . $current_user_id);
    update_field('my_pitch_2', $custom_2, 'user_' . $current_user_id);
    update_field('my_pitch_3', $custom_3, 'user_' . $current_user_id);
    update_field('my_pitch_4', $custom_4, 'user_' . $current_user_id);
    update_field('my_pitch_5', $custom_5, 'user_' . $current_user_id);
    update_field('my_pitch_6', $custom_6, 'user_' . $current_user_id);
    update_field('my_pitch_7', $custom_7, 'user_' . $current_user_id);
    update_field('my_pitch_8', $custom_8, 'user_' . $current_user_id);
    update_field('my_pitch_9', $custom_9, 'user_' . $current_user_id);
    update_field('my_pitch_10', $custom_10, 'user_' . $current_user_id);
